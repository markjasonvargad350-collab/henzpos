...
export const UnifiedDatabaseModal: React.FC = () => {
  ...
  const djabezStockTotal = products.reduce((acc, p) => acc + p.stockUsaBranch, 0);
  const networkStockTotal = mainStockTotal + djabezStockTotal;
  ...
  const djabezTransactions = transactions.filter((t) => t.branch === BRANCH_DJABEZ);

  const mainPreOrders = preOrders.filter((po) => po.pickupBranch === BRANCH_MAIN);
  const djabezPreOrders = preOrders.filter((po) => po.pickupBranch === BRANCH_DJABEZ);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-300 mt-0.5">
                Single unified relational state powering both Main Branch (Casa Conching) and D&apos;Jabez Branch (Gen. Luna St)
              </p>
            </div>
          </div>
              ...
              <div className="bg-slate-900 text-white rounded-xl p-5 border border-slate-800 relative overflow-hidden">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
                  ...
                  <div className="bg-slate-800/80 border border-slate-700 rounded-lg p-4 text-left">
                    ...
                    <h4 className="font-bold text-sm text-white">D&apos;Jabez Bldg.</h4>
                    <p className="text-xs text-slate-400 mb-3">21 Gen. Luna St, Iloilo City Proper</p>
                    <div className="space-y-1 text-xs border-t border-slate-700/60 pt-2 text-slate-300">
                      <div className="flex justify-between">
                        <span>Branch Stock:</span>
                        <span className="font-bold text-emerald-400">{djabezStockTotal.toLocaleString()} units</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Sales Completed:</span>
                        <span className="font-semibold text-white">{djabezTransactions.length}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Assigned Pre-Orders:</span>
                        <span className="font-semibold text-white">{djabezPreOrders.length}</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5">
                  ...
                  <div className="text-xs text-slate-500 mt-0.5">
                    {mainStockTotal} Main / {djabezStockTotal} D&apos;Jabez
                  </div>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    Audit log of stock transfers between Casa Conching (Main) and D&apos;Jabez (21 Gen. Luna St)
                  </p>
                </div>
              ...
              <div className="bg-red-50/60 border border-red-200 rounded-xl p-5 flex items-start justify-between">
                <div>
                  ...
                  <p className="text-xs text-red-700 mt-1 max-w-lg">
                    Restores the catalog to the default standard medical items with clean stock distribution between the Casa Conching and D&apos;Jabez branches. Sales records are left untouched.
                  </p>
                </div>
        ...
        <div className="bg-slate-100 border-t border-slate-200 p-4 px-6 flex items-center justify-between text-xs text-slate-500">
          <div className="flex items-center gap-4">
            <span className="flex items-center gap-1.5 font-medium text-slate-700">
              ...
              Branch 1: Casa Conching (Jalandoni St)
            </span>
            <span className="flex items-center gap-1.5 font-medium text-slate-700">
              <Building2 className="w-3.5 h-3.5 text-emerald-600" />
              Branch 2: D&apos;Jabez (Gen. Luna St)
            </span>
          </div>
