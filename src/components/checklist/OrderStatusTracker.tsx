      ...
      {selectedOrder ? (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-5 border-b border-slate-800">
            ...
            <div className="bg-slate-950/80 border border-slate-800 rounded-xl p-3 text-xs space-y-1.5 md:text-right">
              <div className="flex items-center md:justify-end gap-1.5 text-teal-400 font-semibold">
                <Building2 className="w-4 h-4" />
                <span>Pickup Counter:</span>
              </div>
              <p className="text-white font-medium text-sm">{selectedOrder.pickupBranch}</p>
              <p className="text-slate-400 text-[11px] flex items-center md:justify-end gap-1">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <StepIcon
                        className={`w-4 h-4 ${
                          isCurrent ? 'text-teal-400' : isCompleted ? 'text-emerald-400' : 'text-slate-600'
                        }`}
                      />
                    </div>
                    <p
                      className={`text-xs font-bold ${
                        isCurrent ? 'text-teal-300' : isCompleted ? 'text-white' : 'text-slate-400'
                      }`}
                    >
                      {s.title}
                    </p>
          ...
          {selectedOrder.orderStatus === 'Ready for Pickup' && (
            <div className="bg-emerald-950/50 border border-emerald-500/40 rounded-xl p-4 flex items-start gap-3 text-emerald-200">
          ...
          {selectedOrder.orderStatus === 'Preparing' && (
            <div className="bg-sky-950/40 border border-sky-500/30 rounded-xl p-4 flex items-start gap-3 text-sky-200">
              <PackageCheck className="w-5 h-5 text-sky-400 flex-shrink-0 mt-0.5 animate-bounce" />
              <div className="text-xs space-y-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 pt-2">
            ...
            <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-4 flex flex-col items-center justify-center text-center">
              ...
              <p className="font-mono text-xs font-bold text-teal-400 mt-2">
                {selectedOrder.orderNumber}
              </p>
              ...
              <button
                ...
                className="mt-3 px-3 py-1.5 text-xs font-medium bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-lg transition cursor-pointer flex items-center gap-1.5"
              >
          ...
          {onNewOrderClick && (
            <div className="pt-2 flex justify-between items-center text-xs">
              <button
                ...
                className="text-teal-400 hover:text-teal-300 font-medium transition flex items-center gap-1 cursor-pointer"
              >
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 text-center space-y-3">
          ...
          <p className="text-xs text-slate-400 max-w-md mx-auto">
            We couldn't find an active order matching "<span className="text-white font-semibold">{searchQuery}</span>". Please verify your order number (e.g. HENZ-ORD-2026-XXXX) or try searching with your full name.
          </p>
          {onNewOrderClick && (
            <button
              onClick={onNewOrderClick}
